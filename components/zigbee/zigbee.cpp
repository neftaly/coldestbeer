...
void ZigBeeComponent::setup() {
  ESP_LOGI(TAG, "Zigbee setup starting");
  global_zigbee = this;
...
#ifdef CONFIG_WIFI_COEX
  ESP_LOGI(TAG, "Enabling WiFi/802.15.4 coexistence");
  if (esp_coex_wifi_i154_enable() != ESP_OK) {
    ESP_LOGE(TAG, "Failed to enable WiFi/802.15.4 coexistence");
    this->mark_failed();
    return;
  }
#endif
  // ESP_ERROR_CHECK(nvs_flash_init()); not needed, called by esp32 component
  esp_err_t platform_ret = esp_zb_platform_config(&config);
  if (platform_ret != ESP_OK) {
    ESP_LOGE(TAG, "Failed to configure Zigbee platform: %s", esp_err_to_name(platform_ret));
    this->mark_failed();
    return;
  }
  ESP_LOGI(TAG, "Platform configured OK");
  ...
  ESP_LOGI(TAG, "Zigbee stack initialized, role=%d, endpoints=%d", this->device_role_, this->endpoint_list_.size());
  ...
  ESP_LOGI(TAG, "Creating Zigbee task (stack=4096, priority=24)");
  BaseType_t task_ret = xTaskCreate(esp_zb_task_, "Zigbee_main", 4096, NULL, 24, NULL);
  if (task_ret != pdPASS) {
    ESP_LOGE(TAG, "Failed to create Zigbee task: %d", task_ret);
    this->mark_failed();
    return;
  }
  ESP_LOGI(TAG, "Zigbee setup complete");
